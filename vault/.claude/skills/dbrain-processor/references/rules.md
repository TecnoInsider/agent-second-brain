...
1. **Двигает к $3K/мес и Валенсии?** Нет → понизить приоритет или → идеи без срока
2. **Сколько активных проектов?** Если 3 → новый проект НЕ создаётся → идеи без срока
...
**ВСЕГДА трансформируй:**
- Цели-результаты → Цели-действия (см. process-goals.md)
- "Надо сделать X" → конкретный первый шаг с датой и временем
- "Классная идея!" → записать в идеи, проверить через 24 часа
...
- [ ] Все задачи имеют даты и конкретные действия (цели-действия, не результаты)
